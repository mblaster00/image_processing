...
class GenNeirest:
    ...
    def generate(self, ske):
        ...
        for i in range(self.videoSkeletonTarget.skeCount()):  # Use skeCount to iterate over indices
            target_skeleton = self.videoSkeletonTarget.ske[i]  # Get the target skeleton
            target_image = self.videoSkeletonTarget.imagePath(i)  # Get the corresponding image path

            # Assuming you have a method to load the image
            loaded_image = Image.open(target_image)
            loaded_image = np.array(loaded_image)  # Convert to NumPy array if necessary

            distance = self.euclidean_distance(ske, target_skeleton)

            if distance < min_distance:
                min_distance = distance
                nearest_image = loaded_image  # Store the corresponding image
        ...
        empty = np.ones((64, 64, 3), dtype=np.uint8) * 255  # A white blank image
        return empty